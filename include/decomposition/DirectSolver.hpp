...
std::unique_ptr<DecomposeResult<datatype> > evd(DenseTensor<datatype, 2, computEnv, maptype> tensor){
    ...
    auto eigvec_0 = new datatype[n*n];
    auto eigvec_1 = new datatype[n*n];
    
    int info = geev<datatype, computEnv>(SE_layout::ColMajor, 'V', 'V', n, tensor.data, n, real_eigvals.get(), imag_eigvals.get(), eigvec_0, n, eigvec_1, n);
    //Check for convergence
    if( info > 0 ) {
        printf( "The algorithm failed to compute eigenvalues.\n" );
        exit( 1 );
    }
    eigenvec_sort<datatype, computEnv>(real_eigvals.get(), eigvec_0, n,n);
    //Print eigenvalues
