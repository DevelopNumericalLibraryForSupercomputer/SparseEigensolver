...
std::unique_ptr<DecomposeResult<DATATYPE> > evd(DenseTensor<2,DATATYPE,mtype,device>& tensor, DenseTensor<2, DATATYPE, mtype, device>* eigvec){assert(false); return NULL;};
...
std::unique_ptr<DecomposeResult<DATATYPE> > evd(DenseTensor<2,DATATYPE,mtype,DEVICETYPE::MKL>& tensor, DenseTensor<2, DATATYPE, mtype, DEVICETYPE::MKL>* eigvec){
    ...
    assert(mtype==MTYPE::Contiguous1D);
...
template <typename DATATYPE, MTYPE mtype>
std::unique_ptr<DecomposeResult<DATATYPE> > evd(DenseTensor<2,DATATYPE,mtype,DEVICETYPE::SYCL>& tensor, DenseTensor<2, DATATYPE, mtype, DEVICETYPE::SYCL>* eigvec){
    DEVICETYPE device = DEVICETYPE::SYCL;
    using REALTYPE = typename real_type<DATATYPE>::type;

    assert(mtype==MTYPE::Contiguous1DCol);
    assert(tensor.ptr_map->get_global_shape()[0] == tensor.ptr_map->get_global_shape()[1]);
    const int n = tensor.ptr_map->get_global_shape()[0];
    
    std::complex<REALTYPE>* eigvals_ptr = malloc<std::complex<REALTYPE>, device> (n);
    //std::unique_ptr<std::complex<REALTYPE>[]> eigvals_ptr(new std::complex<REALTYPE>[n]);
    std::unique_ptr<REALTYPE[], std::function<void(REALTYPE*)>> real_eigvals_ptr( malloc<DATATYPE,device>(n), free<device>);
    
    std::unique_ptr<DATATYPE[],std::function<void(DATATYPE*)>> left_eigvec( malloc<DATATYPE,device>( n*n),free<device>  );
    std::unique_ptr<DATATYPE[],std::function<void(DATATYPE*)>> right_eigvec( malloc<DATATYPE,device>(n*n), free<device> );
/*
    double* mat_new = new double[n*n];
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
