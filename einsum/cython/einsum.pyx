...
#ctypedef np.uint64_t DTYPE_t #unsigned long long
...
cdef extern from "einsum_vector.hpp":
    ...
    #void einsum(const string input_str, const double* a, const double* b, double* C, vector[size_t] a_size, vector[size_t] b_size)
...
def c_einsum(str input_string, np.ndarray A, np.ndarray B):
    ...
    cdef double[::1] B_view = np.ravel(B, order='C')  # Use C-contiguous view
    cdef double[::1] C_view = np.ravel(new_C, order='C')
    #einsum(<string>input_string.encode('utf-8'), &A_view[0], &B_view[0], &C_view[0], a_size, b_size)
    einsum_core(result_index, inputs_exprs, total_iter_sizes, &A_view[0], &B_view[0], &C_view[0], a_size, b_size, c_size)
    #new_C = np.zeros((3,2,1), dtype=np.float64)
    return new_C
