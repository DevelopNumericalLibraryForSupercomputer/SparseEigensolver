...
#include "device/sycl/TensorOp.hpp"
#include "device/sycl/SYCLComm.hpp"
#define  MAPTYPE SE::MTYPE::Contiguous1DCol
#define  DTYPE SE::DEVICETYPE::SYCL
//#include "device/mpi/TensorOp.hpp"
//#include "device/mpi/MPIComm.hpp"
//#define  MAPTYPE SE::MTYPE::Contiguous1D
//#define  DTYPE SE::DEVICETYPE::MPI
...
int main(int argc, char* argv[]){
    ...
    SYCLCommInp comm_inp(get_device(0));
    ...
    Contiguous1DColMapInp<2> map2_inp( shape );

    //1D Hamiltonian-like matrix, DenseTensor
    DATATYPE invh2 = 1.0;
    std::unique_ptr<DATATYPE[], std::function<void(DATATYPE*)> > matrix_data ( malloc<DATATYPE, DEVICETYPE::MKL>(N*N), free<DEVICETYPE::MKL> );
    for(int i=0;i<N;i++){
        for(int j=0;j<N;j++){
            matrix_data.get()[i+j*N] = 0;
            if(i == j)                  matrix_data.get()[i+j*N] += 2.0*((DATATYPE)i-(DATATYPE)N)   - invh2*5.0/2.0;
            if(is_complex_v<DATATYPE>){
                //if(i == j + 1 )  matrix_data.get()[i+j*N] += std::complex(std::real(invh2)*4.0/3.0,1.0);
                //if(i == j - 1 )  matrix_data.get()[i+j*N] += std::complex(std::real(invh2)*4.0/3.0,-1.0);
            }
            else{
    ...
    DenseTensor<2,DATATYPE,MAPTYPE,DTYPE> test_matrix(ptr_comm, map2_inp.create_map(), std::move(matrix_data));

//    //SparseTensor which is the same as the DenseTensor
//    SE::SparseTensor<2, DATATYPE, MAPTYPE, DTYPE> test_sparse( ptr_comm, map2_inp.create_map(), N*3);
//    for(int i=0;i<N;i++){
//        std::cout << 1 << "\t" << i <<std::endl;
//        for(int j=0;j<N;j++){
//            std::array<int,2> index = {i,j};
//            if(i == j)                   test_sparse.global_insert_value(index, 2.0*((DATATYPE)i-(DATATYPE)N)   - invh2*5.0/2.0);
//            if(is_complex_v<DATATYPE>){
//                if(i == j +1)   test_sparse.global_insert_value(index, std::complex<double>(invh2.real()*4.0/3.0,1.0));
//                if(i == j -1)   test_sparse.global_insert_value(index, std::complex<double>(invh2.real()*4.0/3.0,-1.0));
//            }
//            else{
...
//    std::cout << "# Matrix construction complete" << std::endl;
    ...
    Contiguous1DColMapInp<2> guess_map_inp( guess_shape );
    std::unique_ptr<Map<2,MAPTYPE> > ptr_guess_map = guess_map_inp.create_map();

    auto ptr_guess1 = std::make_unique< DenseTensor<2, DATATYPE, MAPTYPE, DTYPE> >(ptr_comm, ptr_guess_map);
    // guess : unit vectors
    for(int i=0;i<num_eig;i++){
        std::array<int, 2> tmp_index = {i,i};
        ptr_guess1->global_set_value(tmp_index, 1.0);
    }
    auto ptr_guess2 = std::make_unique< DenseTensor<2, DATATYPE, MAPTYPE, DTYPE> >(*ptr_guess1);
    auto ptr_guess3 = std::make_unique< DenseTensor<2, DATATYPE, MAPTYPE, DTYPE> >(*ptr_guess1);
    auto ptr_guess4 = std::make_unique< DenseTensor<2, DATATYPE, MAPTYPE, DTYPE> >(*ptr_guess1);
    auto ptr_guess5 = std::make_unique< DenseTensor<2, DATATYPE, MAPTYPE, DTYPE> >(*ptr_guess1);
    ...
    print_eigenvalues( "Eigenvalues", num_eig, out_direct->real_eigvals.get(), out_direct->imag_eigvals.get());
    ...
    //option_evd.preconditioner    = PRECOND_TYPE::Diagonal;
    option_evd.algorithm_type = DecomposeMethod::Davidson;
    option_evd.num_eigenvalues = num_eig;
    std::cout << "\n2-1. Matrix is given by DenseTensor" << std::endl;
    std::cout << "========================\nDense matrix diag start" << std::endl;
    std::chrono::steady_clock::time_point begin2 = std::chrono::steady_clock::now();  
    auto out2 = decompose(test_matrix, ptr_guess2.get(), option_evd);
    std::chrono::steady_clock::time_point end2 = std::chrono::steady_clock::now();
    std::cout << "========================\nDense matrix diag done" << std::endl; 
    print_eigenvalues( "Eigenvalues", num_eig, out2->real_eigvals.get(), out2->imag_eigvals.get());
...
////    auto out3 = decompose(test_sparse, ptr_guess3.get(), option_evd);
