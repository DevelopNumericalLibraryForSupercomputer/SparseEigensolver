...
//        const int i_negone = -1;
//        auto tmp_out = malloc<double,DEVICETYPE::MPI> (m*n);
//        memset<double,DEVICETYPE::MPI>(tmp_out, 0, m*n);
        ...
        for (int a=0; a<m;a++){
            for(int b=0; b<n; b++){
                for (int c=0; c<k;c++){
                    output->data[a+b*m] += this->matrix_elements[a*m+c]*vec.data[c+b*k];
                }
            }
        }
//        #pragma omp parallel for collapse(2)
...
//            for(int b=0; b<n; b++){
//                for (int c=0; c<k;c++){
//                    tmp_out[a+b*m] += this->matrix_elements[a*m+c]*vec.data[c+b*k];
//                }
//            }
//        }
//        const auto num_local_element = output->ptr_map->get_num_local_elements();
//        auto tmp_out = malloc<double,DEVICETYPE::MPI> (m*n);
//        memset<double,DEVICETYPE::MPI>(tmp_out, 0, m*n);
//        gsum2d<double>(&ictxt, "C", "1-tree", &mat_size, &n, tmp_out, &m, &i_negone, &i_negone);
//        #pragma omp parallel for collapse(2)
//        for (int b=0; b<n;b++){
//            for (int a=0; a<m;a++){
//                output->global_set_value(std::array<int,2>({a,b}), tmp_out[a+b*m] );
//            }
//        }
//        free<DEVICETYPE::MPI>(tmp_out);
