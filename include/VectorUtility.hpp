...
void eigenvec_sort(typename real_type<DATATYPE>::type* eigvals, DATATYPE* eigvecs, const int number_of_eigvals, const int vector_size){
    ...
    REALTYPE* new_eigvals = malloc<REALTYPE,device>(number_of_eigvals);
    DATATYPE* new_eigvecs = malloc<DATATYPE,device>(number_of_eigvals*vector_size);
    ...
    free<device>( new_eigvals ); 
    free<device>( new_eigvecs);
}
