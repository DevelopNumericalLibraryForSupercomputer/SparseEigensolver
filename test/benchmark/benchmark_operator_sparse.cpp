...
using COOEntry = std::tuple<int, int, double>;

void readMatrixFromBinaryFile(const std::string& inputFile, std::vector<COOEntry>& entries, std::vector<double>& diag_elements, int& rows, int& cols) {
    ...
    // Read data
    entries.resize(nnz);
    infile.read(reinterpret_cast<char*>(entries.data()),sizeof(COOEntry)*nnz);
...
//        entries[i] = std::make_tuple(row, col, value);
        if(std::get<0>(entries[i])==std::get<1>(entries[i])){
            diag_elements.push_back(value);
        }
    }
...
class Operations: public TensorOperations<double, MTYPE::BlockCycling, DEVICETYPE::MPI> {
    public:
        Operations(int mat_size, COOEntry* matrix_elements, double* diag_elements):mat_size(mat_size),matrix_elements(matrix_elements), diag_elements(diag_elements){
            SE::ElapsedTime::time_record["custom_matmul"]= std::vector<double>();
            return;         
        };
        ...
        std::unique_ptr<DenseTensor<2, double, MTYPE::BlockCycling, DEVICETYPE::MPI> > matvec(const DenseTensor<2, double, MTYPE::BlockCycling, DEVICETYPE::MPI>& vec) const{
            ...
            const int k = vec.ptr_map->get_local_shape(0);
            ...
            for (int c=0; c<k; c++){
                for (int b=0; b<n; b++){
                    std::cout << vec.data[c+b*k] <<"\t";
                }
                std::cout << std::endl;
            }
            
            auto matrix_element_end = matrix_elements+mat_size;
            for (int b=0; b<n; b++){
                #pragma omp parallel for
                for (auto iter=matrix_elements; iter!=matrix_element_end; iter++){
                    const auto a= std::get<0>(*iter);
                    const auto c= std::get<1>(*iter);
                    const auto val= std::get<2>(*iter);

                    output->data[a+b*m]+=vec.data[c+b*k]*val;
                }
            }
            std::cout  << "============ out " <<std::endl;
            for (int a=0; a<m; a++){
                for (int b=0; b<n; b++){
                    std::cout << vec.data[a+b*k] <<"\t";
                }
                std::cout <<std::endl;
            }
            std::chrono::steady_clock::time_point end0 = std::chrono::steady_clock::now();
            SE::ElapsedTime::time_record["custom_matmul"].push_back( ((double)std::chrono::duration_cast<std::chrono::microseconds>(end0 - begin0).count())/1000000.0 );
            //std::cout << "time("<<m<<","<<n<<","<<k<<"): " <<((double)std::chrono::duration_cast<std::chrono::microseconds>(end0 - begin0).count())/1000000.0 <<std::endl;
...
int main(int argc, char** argv){
    ...
    option.num_eigenvalues = 256;
    option.max_block = 3;
    option.max_iterations = 250;
    option.preconditioner_max_iterations = 15;
    ...
    switch (file_number){
        case 0:
            filename = "Sparse_matrix/3223_0.dat";
            break;
        case 1:
            filename = "Sparse_matrix/2222_0.dat";
            break;
        case 3:
            filename = "Sparse_matrix/tiny.dat";
    }
    if (argc>=8 ) option.num_eigenvalues = std::stoi(argv[7]);
    ...
    std::chrono::steady_clock::time_point begin0 = std::chrono::steady_clock::now();  
    int rows, cols;
    std::vector<COOEntry> output;
    std::vector<double> diag_elements;
    
    readMatrixFromBinaryFile(filename, output, diag_elements, rows, cols);

    const int N = rows;
    assert ( rows==cols);
    //printMatrix(output.second, N, N);
    std::chrono::steady_clock::time_point end0 = std::chrono::steady_clock::now();
    if(ptr_comm->get_rank()==0) std::cout << "reading matrix takes(" << N <<") " << ((double)std::chrono::duration_cast<std::chrono::microseconds>(end0 - begin0).count())/1000000.0 << "[sec]" << std::endl;
    ...
    Operations op(N, output.data(), diag_elements.data());
    ...
    std::cout << "test local size" <<  guess->ptr_map->get_local_shape(0) <<"/" << guess->ptr_map->get_local_shape(1) <<std::endl;
    //    // guess : unit vector
    ...
    try{
        ...
        if(ptr_comm->get_rank()==0) print_eigenvalues( "Eigenvalues", option.num_eigenvalues, out1->real_eigvals.get(), out1->imag_eigvals.get());
    }
