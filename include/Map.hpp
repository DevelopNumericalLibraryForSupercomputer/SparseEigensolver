...
public:
	...
	virtual size_t get_start_my_global_index(size_t rank) = 0;
	virtual size_t get_end_my_global_index(size_t rank) = 0;
	//virtual size_t get_end_my_local_index(size_t slice_dimension, size_t rank) = 0;
	virtual size_t get_my_partition_size(size_t rank) = 0;
	virtual size_t* get_partition_size_array() = 0;
	virtual size_t* get_start_global_index_array() = 0;

	virtual size_t get_my_rank_from_global_index(const size_t global_index) = 0;
	virtual size_t get_my_rank_from_global_index(const std::array<size_t, dimension> global_index) = 0;
	//virtual size_t calculate_chunk_size(size_t num_global_index, size_t world_size) = 0;
	
	// array -> array
	virtual std::array<size_t, dimension> get_global_array_index(const std::array<size_t, dimension> local_index, size_t rank) = 0;
	virtual std::array<size_t, dimension> get_local_array_index (const std::array<size_t, dimension> global_index, size_t rank) = 0;
	// size_t -> array
	virtual std::array<size_t, dimension> get_global_array_index(const size_t local_index, size_t rank) = 0;
	virtual std::array<size_t, dimension> get_local_array_index (const size_t global_index, size_t rank) = 0;
	// array -> size_t
	virtual size_t get_global_index(const std::array<size_t, dimension> local_index, size_t rank) = 0;
	virtual size_t get_local_index(const std::array<size_t, dimension> global_index, size_t rank) = 0;
	// size_t -> size_t
	virtual size_t get_global_index(const size_t local_index, size_t rank) = 0;
	virtual size_t get_local_index(const size_t global_index, size_t rank) = 0;
